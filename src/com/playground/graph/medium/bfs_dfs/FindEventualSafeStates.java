...
public class FindEventualSafeStates {
    ...
    /**
     * Runtime 4 ms Beats 98.28%
     * Memory 54.7 MB Beats 61.59%
     */
    public List<Integer> eventualSafeNodes(int[][] graph) {
        Boolean[] isSafe = new Boolean[graph.length];

        for(int node = 0; node < graph.length; node++)
            if(isSafe[node] == null)  eventualSafeNodesDfs(graph, isSafe, node);


        List<Integer> res = new ArrayList<>();
        for(int node = 0; node < graph.length; node++) if(isSafe[node]) res.add(node);

        return res;
    }

    public boolean eventualSafeNodesDfs(int[][] graph, Boolean[]isSafe, int node) {
        if(isSafe[node] != null) return isSafe[node];

        isSafe[node] = false;
        for(int neighbor: graph[node]) {
            boolean isNeighborSafe = eventualSafeNodesDfs(graph, isSafe, neighbor);
            if(!isNeighborSafe) return false;
        }
        isSafe[node] = true;
        return true;
    }
